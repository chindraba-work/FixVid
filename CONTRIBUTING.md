...
This project started from a personal need to modify many (hundreds) of videos to make them usable. Major emphasis on enhancing the audio to be near full-scale sound levels with reduced range between dialog and sound effects (especially when dialog is soft and sound effects are loud). Adding in subtitles when available helps cover for areas of sound that are not practical to adjust. Reducing the file size by re-encoding the video is a nice benefit, but not the motivation.

Doing all the work by hand was very tedious. Using copy & paste, and then a single script that was continually re-edited for each video improved the process marginally. Creating a more robust alternative with details placed in a configuration file and the "work" into stable scripts became the genesis for this project. Therein lies the real goal, and objective, with anything else being "extras" that might be worthy, and yet are neglected or postponed until later.
